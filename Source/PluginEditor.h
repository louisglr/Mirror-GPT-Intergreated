...
private:
    ...
    void lockKnob(KnobWithLabel& k, const juce::String& lockedLabel, const juce::String& helpText);
    ...
    juce::ComboBox vocalRangeBox, harmonyStyleBox;
    juce::Label modeLabel, vocalRangeLabel, harmonyStyleLabel;
    std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> modeAttachment, vocalRangeAttachment, harmonyStyleAttachment;
    ...
    // MIDI-only controls replace the Manual-only input controls while MIDI mode
    // is selected.  Their parameters are intentionally kept in the processor
    // so they remain host-automatable and session-safe.
    KnobWithLabel midiVelocityKnob;
    juce::ComboBox midiVoicingBox, midiInversionBox, midiTimingBox;
    juce::Label midiVoicingLabel, midiInversionLabel, midiTimingLabel;
    std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> midiVoicingAttachment, midiInversionAttachment, midiTimingAttachment;
    ...
    // Preset names are an editor convenience rather than an audio parameter.
    // A restored session therefore starts in the honest Custom/Select state.
