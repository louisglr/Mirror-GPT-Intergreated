...
void MirrorAudioProcessor::cacheParameterPointers()
{
    ...
    parameterValues.midiTiming = get("midiTiming");
...
juce::AudioProcessorValueTreeState::ParameterLayout MirrorAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>(
        juce::ParameterID{ "midiTiming", 1 }, "MIDI Timing",
        juce::StringArray{ "Live", "Aligned" }, 0));
...
void MirrorAudioProcessor::prepareToPlay(double sampleRate, int)
{
    ...
    midiPendingNotes.fill(69);
    midiPendingVelocities.fill(0.0f);
    midiPendingFrequencies.fill(440.0f);
    ...
    // Exact linear ramp lengths: unlike an exponential time constant, these
    // reach zero/one in the named time and do not turn a 7 ms fade into a
    // hidden 40+ ms MIDI delay.
    ...
    const std::array<bool, kNumHarmonyVoices>& participatingVoices)
{
    ...
    for (int inversion = firstInversion; inversion <= lastInversion; ++inversion)
    {
        ...
        // Voicing must reshape the chord the player actually supplied, not
        // only extra duplicated tones.  That made Open/Wide sound identical
        // whenever four notes drove four voices.  Open uses a musical drop-2
        // layout; Wide separates the lower and upper halves by an octave.
        if (midiVoicing == 1) // Open
        {
            if (numHeldNotes >= 3)
                chordNotes[(size_t) (numHeldNotes - 2)] -= 12;
            else if (numHeldNotes == 2)
                chordNotes[1] += 12;
        }
        else if (midiVoicing == 2) // Wide
        {
            const int upperHalf = (numHeldNotes + 1) / 2;
            for (int j = 0; j < numHeldNotes; ++j)
                chordNotes[(size_t) j] += j < upperHalf ? -12 : 12;
        }

        for (int j = 0; j < numHeldNotes; ++j)
            chordNotes[(size_t) j] = juce::jlimit(24, 108, chordNotes[(size_t) j]);

        // Restore vertical order after register transformations while keeping
        // every note's velocity attached to it.
        for (int j = 1; j < numHeldNotes; ++j)
        {
            const int note = chordNotes[(size_t) j];
            const float velocity = chordVelocities[(size_t) j];
            int destination = j;
            while (destination > 0 && chordNotes[(size_t) (destination - 1)] > note)
            {
                chordNotes[(size_t) destination] = chordNotes[(size_t) (destination - 1)];
                chordVelocities[(size_t) destination] = chordVelocities[(size_t) (destination - 1)];
                --destination;
            }
            chordNotes[(size_t) destination] = note;
            chordVelocities[(size_t) destination] = velocity;
        }

        // Extra voices repeat chord tones above the chosen Open/Wide layout.
        ...
        for (int p = 0; p < paletteCount; ++p)
        {
            ...
            paletteNotes[(size_t) p] = juce::jlimit(24, 108,
                chordNotes[(size_t) chordIndex] + 12 * octave);
    ...
    for (int d = 0; d < activeCount; ++d)
    {
        ...
        requestMidiAssignment(voice, bestPaletteNotes[(size_t) paletteIndex],
                              bestPaletteVelocities[(size_t) paletteIndex]);
    }
}

void MirrorAudioProcessor::requestMidiAssignment(int voice, int note, float velocity)
{
    const size_t index = (size_t) juce::jlimit(0, kNumHarmonyVoices - 1, voice);
    note = juce::jlimit(0, 127, note);
    velocity = juce::jlimit(0.0f, 1.0f, velocity);

    if (!midiVoiceHasTargets[index])
    {
        midiAssignedNotes[index] = note;
        midiAssignedVelocities[index] = velocity;
        midiAssignedFrequencies[index] = PitchCorrector::midiToFreq(note);
        voiceLastMidi[index] = (float) note;
        midiVoiceHasTargets[index] = true;
        midiPendingTargets[index] = false;
        midiRetargetGains[index] = 1.0f;
        return;
    }

    if (!midiPendingTargets[index] && midiAssignedNotes[index] == note)
    {
        midiAssignedVelocities[index] = velocity;
        return;
    }

    if (midiPendingTargets[index] && midiPendingNotes[index] == note)
    {
        midiPendingVelocities[index] = velocity;
        return;
    }

    // Retarget only after a short fade to zero.  A direct change moves an
    // active granular reader discontinuously and is heard as a synthetic
    // click/zipper when MIDI chords change or individual notes are released.
    midiPendingNotes[index] = note;
    midiPendingVelocities[index] = velocity;
    midiPendingFrequencies[index] = PitchCorrector::midiToFreq(note);
...
void MirrorAudioProcessor::enqueueMidiMessage(const juce::MidiMessage& message, int sampleOffset)
{
    ...
    while (insertion > 0
           && queuedMidiEvents[(size_t) (insertion - 1)].sampleOffset > event.sampleOffset)
    {
        queuedMidiEvents[(size_t) insertion] = queuedMidiEvents[(size_t) (insertion - 1)];
        --insertion;
    }
    queuedMidiEvents[(size_t) insertion] = event;
...
{
    int dueCount = 0;
    while (dueCount < queuedMidiEventCount
           && queuedMidiEvents[(size_t) dueCount].sampleOffset <= sampleOffset)
    {
        handleQueuedMidiEvent(queuedMidiEvents[(size_t) dueCount]);
        ++dueCount;
    }
    ...
    for (int i = dueCount; i < queuedMidiEventCount; ++i)
        queuedMidiEvents[(size_t) (i - dueCount)] = queuedMidiEvents[(size_t) i];
    queuedMidiEventCount -= dueCount;
}

void MirrorAudioProcessor::advanceQueuedMidiEvents(int blockSize)
{
    for (int i = 0; i < queuedMidiEventCount; ++i)
        queuedMidiEvents[(size_t) i].sampleOffset -= blockSize;
}

bool MirrorAudioProcessor::detectTransportDiscontinuity(int blockSize)
{
    auto* playHead = getPlayHead();
    if (playHead == nullptr)
    {
        hasHostTransportPosition = false;
        lastHostBlockSize = blockSize;
        return false;
    }
    ...
    if (hasHostTransportPosition)
    {
        const std::int64_t expected = lastHostSamplePosition + (std::int64_t) lastHostBlockSize;
        const std::int64_t deviation = *timeInSamples - expected;
        bool seamlessLoopWrap = false;
        if (position->getIsLooping() && deviation < -1 && hasHostPpqPosition
            && ppqPosition.hasValue())
        {
            if (const auto loop = position->getLoopPoints())
            {
                const auto bpmValue = position->getBpm();
                const double bpm = bpmValue.hasValue() ? *bpmValue : 120.0;
                const double blockPpq = (double) juce::jmax(1, lastHostBlockSize)
                    / juce::jmax(1.0, currentSampleRate) * juce::jmax(1.0, bpm) / 60.0;
                const double tolerance = juce::jmax(0.02, blockPpq * 4.0);
                const bool wasNearLoopEnd = lastHostPpqPosition >= loop->ppqEnd - tolerance
                    && lastHostPpqPosition <= loop->ppqEnd + tolerance;
                const bool isNearLoopStart = *ppqPosition >= loop->ppqStart - tolerance
                    && *ppqPosition <= loop->ppqStart + tolerance;
    ...
    lastHostWasPlaying = isPlaying;
    lastHostSamplePosition = *timeInSamples;
    hasHostPpqPosition = ppqPosition.hasValue();
...
void MirrorAudioProcessor::resetProcessingState(bool clearMidiState)
{
    voiceBuffer.reset();
    pitchDetector.reset();
    pitchCorrector.prepare(currentSampleRate);
    dryVoice.reset();
    dryAlignmentL.reset();
    dryAlignmentR.reset();
    ...
    harmonyGlueL.reset();
    harmonyGlueR.reset();
    globalSaturatorL.reset();
    globalSaturatorR.reset();
    ambienceApL1.reset();
    ambienceApL2.reset();
    ambienceApR1.reset();
    ambienceApR2.reset();
...
void MirrorAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    if (detectTransportDiscontinuity(numSamples))
        ...
        // chord whose note-off was skipped by the host can remain stuck. True
        // loop wraps are identified above and retain continuous state.
        resetProcessingState(true);
    ...
    const int midiTiming = (int) parameterValues.midiTiming->load(std::memory_order_relaxed);
    ...
    if (midiTiming != lastMidiTimingMode)
    {
        // A timing-mode switch cannot safely reinterpret events already in
        // flight. Panic only when such a queue exists, preventing a delayed
        // note-on from arriving after the user selected Live.
        if (lastMidiTimingMode >= 0 && queuedMidiEventCount > 0)
        {
            queuedMidiEventCount = 0;
            numHeldNotes = 0;
            physicalKeys.fill(false);
    ...
    for (int i = 0; i < kNumHarmonyVoices; ++i)
    {
        ...
        if (vp[voice].enable && vp[voice].solo) anySolo = true;
    }
    ...
    // corrected manual vocal. Manual is fixed to a responsive but natural
    // 38 ms; the legacy Transition parameter remains registered at 100% for
    // session compatibility without reintroducing the previous 155 ms lag.
    ...
    // Live applies MIDI at its incoming sample position. Aligned intentionally
    // delays chord changes by the granular look-back so they affect the same
    // source syllable. Both paths are fixed-size and allocation-free.
    auto midiEvent = midiMessages.begin();
    const auto midiEnd = midiMessages.end();
    const bool useAlignedMidi = mode == 1 && midiTiming == 1;
    if (useAlignedMidi)
    {
    ...
    for (int n = 0; n < numSamples; ++n)
    {
        ...
        // Pitch value from collapsing a stereo recording to mono without
        ...
        float dryProcL = alignedL + (shiftedL - alignedL) * dryPitchBlend;
        float dryProcR = alignedR + (shiftedR - alignedR) * dryPitchBlend;
        ...
        for (int i = 0; i < kNumHarmonyVoices; ++i)
        {
            ...
            float retargetGain = midiRetargetGains[voice];
            if (!std::isfinite(retargetGain))
                retargetGain = 1.0f;
            if (mode == 1 && midiPendingTargets[voice])
            {
                retargetGain = juce::jmax(0.0f, retargetGain - midiRetargetFadeOutStep);
                if (retargetGain <= 0.0f)
                {
                    midiAssignedNotes[voice] = midiPendingNotes[voice];
                    midiAssignedVelocities[voice] = midiPendingVelocities[voice];
                    midiAssignedFrequencies[voice] = midiPendingFrequencies[voice];
                    voiceLastMidi[voice] = (float) midiAssignedNotes[voice];
                    midiVoiceHasTargets[voice] = true;
                    midiPendingTargets[voice] = false;
                    retargetGain = 0.0f;
                }
            }
            ...
            {
                retargetGain = juce::jmin(1.0f, retargetGain + midiRetargetFadeInStep);
            }
            ...
            raw *= harmonyVoicing * gate * retargetGain;

            const float level = voiceLevelSmoothed[voice].getNextValue();
            const float effectiveGain = juce::jmax(0.0f, level * gate * retargetGain);
            stackGainSum += effectiveGain;
            stackGainSquares += effectiveGain * effectiveGain;
            stackVoiceWeight += gate * retargetGain;
